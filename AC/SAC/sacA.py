...
class SAC:
    ...
    def __init__(self, env, learning_rate_Q_functions=0.01, learning_rate_policy=0.01, gamma=0.99, 
        ...
        #Initiate networks
        self.policy = PolicyNetwork(input_shape=self.observation_shape, arch=[256,256], out_shape=self.action_space,max_action=env.action_space.high)

        self.Q1 = QNetwork(input_shape=self.observation_shape+self.action_space, arch=[256,256])
        self.Q2 = QNetwork(input_shape=self.observation_shape+self.action_space, arch=[256,256])

        self.target_Q1 = QNetwork(input_shape=self.observation_shape+self.action_space, arch=[256,256])
        self.target_Q2 = QNetwork(input_shape=self.observation_shape+self.action_space, arch=[256,256])
        
        #Synchronize target networks and predict networks
        ...
        ## Automatic adjucement of the temperature
        self.H_min = - self.action_space
        self.alpha = 1
        self.log_alpha = torch.tensor([0.0], requires_grad=True)
        
        self.alpha_optimizer = torch.optim.Adam(params=[self.log_alpha], lr=learning_rate_policy)
    ...
    def train(self):
        ...
        #---------TRAINING------------------------
            ...
            target_Qvalue = reward_batch + self.gamma*done_batch*(target_Qvalue -self.alpha*log_prob.squeeze(1))

            ## Computing Q-Value prediction
            ...
            Q1_loss = loss(target_Qvalue, Q1_values)
            Q2_loss = loss(target_Qvalue, Q2_values)   
            ...
            #---- Optimizing Temperature variable
            alpha = torch.exp(self.log_alpha)
            self.alpha_optimizer.zero_grad()
            actions, log_prob = self.policy(state_batch, False)
            
            alpha_loss = (- self.log_alpha*log_prob.detach() - self.log_alpha*self.H_min).mean()
            alpha_loss.backward()
            ...
            ## .... 
            self.losses = (policy_loss.detach().numpy(),  Q1_loss.detach().numpy(), Q2_loss.detach().numpy(), self.alpha)
