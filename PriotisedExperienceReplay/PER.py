...
class Agent:
  def __init__(self, obs_size, n_actions, gamma=0.99, epsilon=0.9, learning_rate=0.001, batch_size=20,
    ...
    self.Q = DuelingNetwork(obs_size, self.n_actions, learning_rate).to(self.device)
    self.target_Q = DuelingNetwork(obs_size, self.n_actions, learning_rate).to(self.device)
  ...
  def train_nn(self):
    ...
    mini_batch, weight = self.replay.get_batch(self.batch_size) # appel remote object method
