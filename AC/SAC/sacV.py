...
class SAC:
    ...
    def __init__(self, env, learning_rate_Q_functions=0.01, learning_rate_policy=0.01, gamma=0.99, 
        ...
        self.V = VNetwork(input_shape=self.observation_shape, hidden_size=256)
        self.target_V = VNetwork(input_shape=self.observation_shape, hidden_size=256)
        
        self.polyak_update(self.V, self.target_V, 1)
        ...
        self.V_optimizer = torch.optim.Adam(params=self.V.parameters(), lr=learning_rate_Q_functions)
    ...
    def train(self):
        ...
        #---------TRAINING------------------------
            ...
            values = self.V(state_batch)
            Q1_values = self.Q1(state_batch, actions)
            Q2_values = self.Q2(state_batch, actions)
            Q_values = torch.min(Q1_values, Q2_values)
            target_values = Q_values-log_prob
            
            loss = torch.nn.MSELoss()
            loss_value = 0.5*loss(values, target_values)
            ...
            values_batch = self.target_V(next_state_batch).squeeze(1)            

            target_Qvalue = (self.scale*reward_batch + self.gamma*done_batch*values_batch)
            loss = torch.nn.MSELoss()
            Q1_loss = 0.5*loss(Q1_values, target_Qvalue)
            Q2_loss = 0.5*loss(Q2_values, target_Qvalue)   
            ...
            self.losses = (policy_loss.detach().numpy(),  Q1_loss.detach().numpy(), Q2_loss.detach().numpy(), loss_value.detach().numpy())

            
            
            self.polyak_update(self.V, self.target_V, self.polyak_factor_Q )